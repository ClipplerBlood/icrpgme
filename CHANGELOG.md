...
- **NEW FEATURE**: Added NEW SHEET DESIGNS
  - Modernized ALL sheets, with more modern and minimalistic design. The new sheets are compatible v13 compatible and
    enabled by default.
  - **IMPORTANT**: Old sheets are still available in the game and can be switched using the sheets' top right actions.
    They will remain available until Foundry v16.
- **BREAKING**: Removed custom UI stylings. This ensures compatibility with FoundryVTT and other external modules while
  looking more modern
...
- Added a **SETTING** to show or hide the "Sheet" header button in actor and item sheets. Useful if you want to use 3rd
  party sheets.

## [1.2.0] Spells and Powers 🪄🦸

- Added **SPELL** item type! Spells are Loots that have additional information such as level, type, duration, target and
  flavor text.
- Added **FILTERS**. In the character sheet loot section, you can press a button to toggle between spells only, loot
  only and loot + spells.
- Improved the **POWER** item type! Powers now have customizable SP costs and a mastery counter each.
- Monster actions and vehicle maneuvers are now clickable. When clicking on the action name, the action gets posted to
  chat.
...
- Added **resource tracking** in Combat Tracker. You can now edit resources such as Mastery, SP or custom ones directly
  from the tracker.
